...
function docrosshairs()
    ...
    for _,v in pairs(game.Workspace.Particles:GetChildren()) do
        if string.find(v.Name, "Crosshair") and v.Parent and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and v.BrickColor ~= BrickColor.new("Flint") then
            if kometa.toggles.fastcrosshairs then
                ...
                if (v.Position - api.humanoidrootpart().Position).magnitude < temptable.magnitude * 0.9 then
                    ...
                    kometa.vars.walkspeed = savespeed * 1.75
                    api.humanoid():MoveTo(v.Position)
                    repeat
                        task.wait()
                    ...
                    kometa.vars.walkspeed = savespeed
...
function UseDispensers()
    ...
    if game.Players.LocalPlayer.Character:FindFirstChild("ProgressLabel",true) then
        local pollenprglbr = game.Players.LocalPlayer.Character:FindFirstChild("ProgressLabel",true)
        maxpoller = tonumber(pollenprglbr.Text:match("%d+$"))
        local pollencounr = game.Players.LocalPlayer.CoreStats.Pollen.Value
        pollenpercentagr = pollencounr/maxpoller*100
    end
    if kometa.toggles.autowreath and tonumber(pollenpercentagr) >= tonumber(kometa.vars.convertat) and CheckToyCooldown("Honey Wreath") and game:GetService("Workspace").Toys['Honey Wreath']:FindFirstChild("ModelAfter") then
    ...
    if kometa.toggles.autosnowmachine and CheckToyCooldown("Snow Machine") and game:GetService("Workspace").Toys['Snow Machine']:FindFirstChild("ModelAfter") then
...
main:Cheat("Label", "Welcome, "..api.nickname.."!")
...
links:Cheat("Button", "Token IDs", function() setclipboard("https://pastebin.com/raw/wtHBD3ij") end, {text="Click to Copy IDs"})
...
farm:Cheat("Checkbox", " ∟ Don't Convert", function(State) kometa.toggles.donotconvert = State end)
farm:Cheat("Checkbox", "Convert Hive Balloon", function(State) kometa.toggles.convertballoons = State end)
farm:Cheat("Checkbox", "Randomize Speed On Autofarming", function(State) kometa.toggles.randomizespeed = State end)
farm:Cheat("Checkbox", "Don't Walk In Field", function(State) kometa.toggles.farmflower = State end)
farm:Cheat("Checkbox", "Don't Farm Tokens", function(State) kometa.toggles.donotfarmtokens = State end)
farm:Cheat("Checkbox", "Honey Mask On Convert", function(State) kometa.toggles.honeymaskconv = State end)
farm:Cheat("Dropdown", " ∟ Default Mask", function(Option) kometa.vars.defmask = Option end, {options = masktable})
local farmsecond = farmtab:Sector("Farm Features")
farmsecond:Cheat("Checkbox", "Auto Sprinkler", function(State) kometa.toggles.autosprinkler = State end)
farmsecond:Cheat("Checkbox", "Auto Dig", function(State) kometa.toggles.autodig = State end)
farmsecond:Cheat("Checkbox", "Auto Mask", function(State) kometa.toggles.automasks = State end)
...
local farmsett = farmtab:Sector("Farm Settings")
farmsett:Cheat("Checkbox", "Farm Bubbles", function(State) kometa.toggles.farmbubbles = State end)
farmsett:Cheat("Checkbox", " ∟ Bubble Bloat Helper", function(State) kometa.toggles.bloatfarm = State end)
farmsett:Cheat("Checkbox", "Farm Flames", function(State) kometa.toggles.farmflame = State end)
...
farmsett:Cheat("Checkbox", " ∟ Smart Precise Crosshairs ⚠️", function(State) kometa.toggles.fastcrosshairs = State end)
...
farmsett:Cheat("Checkbox", "Face Balloons", function(State) kometa.toggles.faceballoons = State end)
local toyfeatures = farmtab:Sector("Toys Features")
toyfeatures:Cheat("Checkbox", "Auto Honeystorm", function(State) kometa.toggles.honeystorm = State end)
toyfeatures:Cheat("Checkbox", "Auto Sprout Summoner", function(State) kometa.toggles.autospawnsprout = State end)
toyfeatures:Cheat("Checkbox", "Auto Wealth Clock", function(State) kometa.toggles.clock = State end)
toyfeatures:Cheat("Checkbox", "Auto Free Ant Pass", function(State) kometa.toggles.freeantpass = State end)
toyfeatures:Cheat("Checkbox", "Auto Free Robo Pass", function(State) kometa.toggles.freerobopass = State end)
local beesmasfeatures = farmtab:Sector("Beesmas Features")
beesmasfeatures:Cheat("Checkbox", "Auto Samovar", function(State) kometa.toggles.autosamovar = State end)
beesmasfeatures:Cheat("Checkbox", "Auto Stockings", function(State) kometa.toggles.autostockings = State end)
beesmasfeatures:Cheat("Checkbox", "Auto Honey Candles", function(State) kometa.toggles.autocandles = State end)
beesmasfeatures:Cheat("Checkbox", "Auto Beesmas Feast", function(State) kometa.toggles.autofeast = State end)
beesmasfeatures:Cheat("Checkbox", "Auto Onett's Lid Art", function(State) kometa.toggles.autoonettart = State end)
beesmasfeatures:Cheat("Checkbox", "Auto Honey Wreath", function(State) kometa.toggles.autowreath = State end)
beesmasfeatures:Cheat("Checkbox", "Auto Snow Machine", function(State) kometa.toggles.autosnowmachine = State end)

local quest1 = questtab:Sector("Auto Quest Settings")
quest1:Cheat("Checkbox", "Auto Accept/Confirm Quests ⚙", function(State) kometa.toggles.autoquest = State end)
quest1:Cheat("Checkbox", "Auto Do Quests ⚙", function(State) kometa.toggles.autodoquest = State end)
quest1:Cheat("Dropdown", "Do NPC Quests", function(Option) kometa.vars.npcprefer = Option end, {options = {'All Quests', 'Bucko Bee', 'Brown Bear', 'Riley Bee', 'Polar Bear'}})
quest1:Cheat("Checkbox", "Teleport To NPC", function(State) kometa.toggles.tptonpc = State end)
local fieldsettings = questtab:Sector("Fields Settings")
...
fieldsettings:Cheat("Button", "Add Field To Blacklist", function() table.insert(kometa.blacklistedfields, temptable.blackfield) game:GetService("CoreGui").kometaUI.Container.Categories.Quests:FindFirstChild("Fields Settings", true).Container["Blacklisted Fields"]:Destroy() fieldsettings:Cheat("Dropdown", "Blacklisted Fields", function(Option) end, {options = kometa.blacklistedfields}) end, {text = ' '})
fieldsettings:Cheat("Button", "Remove Field From Blacklist", function() table.remove(kometa.blacklistedfields, api.tablefind(kometa.blacklistedfields, temptable.blackfield)) game:GetService("CoreGui").kometaUI.Container.Categories.Quests:FindFirstChild("Fields Settings", true).Container["Blacklisted Fields"]:Destroy() fieldsettings:Cheat("Dropdown", "Blacklisted Fields", function(Option) end, {options = kometa.blacklistedfields}) end, {text = ' '})
...
local wayp = misctab:Sector("Waypoints")
...
extras:Cheat("Checkbox", "Walk Speed", function(State) kometa.toggles.loopspeed = State end)
extras:Cheat("Checkbox", "Jump Power", function(State) kometa.toggles.loopjump = State end)
extras:Cheat("Slider", "Walk Speed Value", function(Value) kometa.vars.walkspeed = Value end, {min = 0, max = 120, suffix = " studs", default = 70})
...
local webhooking = extrtab:Sector("Discord Webhooking")
...
raresettings:Cheat("Checkbox", "Enable Token Blacklisting", function(State) kometa.toggles.enabletokenblacklisting = State end)
local dispsettings = setttab:Sector("Auto Dispenser & Boosters Settings")
...
Workspace.Particles.ChildAdded:Connect(function(instance)
    if string.find(instance.Name, "Vicious") then
        ...
        if kometa.webhooking.vicious then api.imagehook(kometawebhook.webhook, "Vicious Bee detected!", "mr nirgga 👴🏿", "https://static.wikia.nocookie.net/bee-swarm-simulator/images/1/1f/Vicious_Bee.png/revision/latest/scale-to-width-down/350?cb=20181130115012&path-prefix=ru") end
    end
end)
...
game:GetService("Workspace").NPCBees.ChildAdded:Connect(function(v)
    if v.Name == "Windy" then
        ...
        if kometa.webhooking.windy then api.imagehook(kometawebhook.webhook, "Windy Bee detected!", "mr nirgga 👴🏿", "https://static.wikia.nocookie.net/bee-swarm-simulator/images/8/85/Windy_Bee.png/revision/latest?cb=20200404000105") end
    end
end)
...
task.spawn(function() while task.wait(.1) do
    if not temptable.converting then
        ...
        if kometa.toggles.autowreath then
...
task.spawn(function() while task.wait(1) do
    ...
    game.CoreGui.kometaUI.Container.Categories.Statistics:FindFirstChild("Statistics", true).Container["Gained Honey"].Title.Text = "🍯 Honey Gained: "..api.suffixstring(temptable.honeycurrent - temptable.honeystart)
    game.CoreGui.kometaUI.Container.Categories.Statistics:FindFirstChild("Statistics", true).Container["Honey Per Hour"].Title.Text = "⏳️ Honey Per Hour: "..api.suffixstring((temptable.honeycurrent - temptable.honeystart)/(temptable.stats.runningfor/3600))
    game.CoreGui.kometaUI.Container.Categories.Statistics:FindFirstChild("Statistics", true).Container["Elapsed Time"].Title.Text = "⏳️ Elapsed Time: "..api.toHMS(temptable.stats.runningfor)
    ...
    game.CoreGui.kometaUI.Container.Categories.Statistics:FindFirstChild("Statistics", true).Container["Killed Windy"].Title.Text = "🌪️ Killed Windy: "..temptable.stats.killedwindy
    game.CoreGui.kometaUI.Container.Categories.Statistics:FindFirstChild("Statistics", true).Container["Killed Vicious"].Title.Text = "⚠️ Killed Vicious: "..temptable.stats.killedvicious
...
game.Players.PlayerAdded:Connect(function(player)
    if kometa.webhooking.playeradded then api.imagehook(kometawebhook.webhook, player.Name.." joined your server", "mr nirgga 👴🏿", "https://www.roblox.com/headshot-thumbnail/image?userId="..player.UserId.."&width=420&height=420&format=png") end
end)
